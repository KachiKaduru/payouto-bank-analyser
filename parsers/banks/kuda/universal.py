...
def parse(path: str) -> List[Dict[str, str]]:
    ...
    try:
                ...
                if tables:
                    for table in tables:
                        ...
                        if not global_headers:
                            print(
                                f"(kuda): No headers found by page {page_num}, skipping table",
                                file=sys.stderr,
                            )
                            continue
                    ...
                    print(
                        f"(kuda): No tables found on page {page_num}, attempting text extraction",
                        file=sys.stderr,
                    )
        ...
        print(f"Error processing kuda statement: {e}", file=sys.stderr)
        return []
