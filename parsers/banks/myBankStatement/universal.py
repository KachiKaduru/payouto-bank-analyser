    ...
    try:
        with pdfplumber.open(path) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                print(f"(myBankStatement): Processing page {page_num}", file=sys.stderr)
                ...
                if tables:
                    for table in tables:
                        ...
                        if not global_headers:
                            print(
                                f"(myBankStatement): No headers found by page {page_num}, skipping table",
                                file=sys.stderr,
                            )
                            continue
                    ...
                    print(
                        f"(myBankStatement): No tables found on page {page_num}",
                        file=sys.stderr,
                    )
        ...
        print(f"Error processing (myBankStatement) statement: {e}", file=sys.stderr)
        return []
