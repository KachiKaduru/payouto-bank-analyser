...
def parse(path: str) -> List[Dict[str, str]]:
    ...
    try:
                ...
                if tables:
                    for table in tables:
                        ...
                        if not global_headers:
                            print(
                                f"(stanbic): No headers found by page {page_num}, skipping table",
                                file=sys.stderr,
                            )
                            continue
                    ...
                    print(
                        f"(stanbic): No tables found on page {page_num}, attempting text extraction",
                        file=sys.stderr,
                    )
        ...
        print(f"Error processing Stanbic Bank statement: {e}", file=sys.stderr)
        return []
