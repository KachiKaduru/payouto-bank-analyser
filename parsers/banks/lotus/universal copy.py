                ...
                if tables:
                    for table in tables:
                        ...
                        if not global_headers:
                            print(
                                f"(lotus): No headers found by page {page_num}, skipping table",
                                file=sys.stderr,
                            )
                            continue
                        ...
                        for row in data_rows:
                            ...
                            # Handle case where both dates are stacked in one cell (Txn + Val)
                            # raw_date = row_dict.get(
                            #     "TXN_DATE", row_dict.get("VAL_DATE", "")
                            # )
                            # if "\n" in raw_date or "\r" in raw_date:
                            #     parts = re.split(r"[\r\n]+", raw_date)
                            #     parts = [p.strip() for p in parts if p.strip()]
                            #     txn_date = (
                            #         normalize_date(parts[0]) if len(parts) > 0 else ""
                            #     )
                            #     val_date = (
                            #         normalize_date(parts[1])
                            #         if len(parts) > 1
                            #         else txn_date
                            #     )
                            # else:
                            ...
                            #         row_dict.get("VAL_DATE", raw_date)
                            #     )
                    ...
                    print(
                        f"(lotus): No tables found on page {page_num}, attempting text extraction",
                        file=sys.stderr,
                    )
        print(
            f"(lotus): Total transactions parsed: {len(transactions)}",
            file=sys.stderr,
        )
        for i, t in enumerate(transactions[:10], 1):  # show first 5
            print(f"(lotus): Sample row {i}: {t}", file=sys.stderr)
