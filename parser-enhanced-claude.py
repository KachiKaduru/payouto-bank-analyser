...
def parse_text_transaction(lines: List[str]) -> Optional[Dict[str, str]]:
    ...
    return {
        ...
        "DEBIT": "0.00",
        "CREDIT": "0.00",
        "BALANCE": f"{amounts[-1]:.2f}" if amounts else "0.00",
        "Check": "",
        "Check 2": "",
    }
...
def calculate_checks(transactions: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """Enhanced balance verification with better error handling"""
    updated = []
    prev_balance = None
    ...
    for i, txn in enumerate(transactions):
        try:
            ...
            if prev_balance is not None and current_balance != 0:
                ...
                txn["Check 2"] = f"{difference:.2f}"

                if not check:
                    logger.warning(
                        f"Balance mismatch at transaction {i+1}: expected {expected}, got {actual}"
                    )
            else:
                txn["Check"] = "TRUE"
                txn["Check 2"] = "0.00"
            ...
            updated.append(txn)
            prev_balance = current_balance if current_balance != 0 else prev_balance

        except Exception as e:
            logger.error(f"Error calculating checks for transaction {i+1}: {e}")
            txn["Check"] = "ERROR"
            txn["Check 2"] = "ERROR"
            updated.append(txn)
...
def parse_text_row(row: List[str], headers: List[str]) -> Dict[str, str]:
    """Parse a row from text extraction"""
    standardized_row = {
        "TXN_DATE": "",
        "VAL_DATE": "",
        "REFERENCE": "",
    ...
    standardized_row.update(
        {
            ...
            "DEBIT": f"{to_float(row_dict.get('DEBIT', '0')):.2f}",
            "CREDIT": f"{to_float(row_dict.get('CREDIT', '0')):.2f}",
            "BALANCE": f"{to_float(row_dict.get('BALANCE', '0')):.2f}",
        }
    )
